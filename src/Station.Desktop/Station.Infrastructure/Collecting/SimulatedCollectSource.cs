...
/// </summary>
public sealed class SimulatedCollectSource : ICollectSource, IRecorderFileStore
{
    ...
    public async Task<int> EraseAsync(CollectDeviceInfo device, IEnumerable<SourceFileInfo> files, CancellationToken cancellationToken)
    {
        await Task.Yield();
        var deleted = 0;
        if (Directory.Exists(_root))
        {
            foreach (var file in files)
            {
                ...
                File.Delete(file.RelativePath); // 普通删除（不覆盖、不粉碎）
                deleted++;
            }
        }
        return deleted;
