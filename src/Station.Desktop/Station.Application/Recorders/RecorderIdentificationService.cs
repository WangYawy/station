...
public sealed class RecorderIdentificationService : IRecorderIdentificationService
{
    ...
    {
        // 不需要根目录配置文件
        if (!_options.EnableProfile)
        {
            return new RecorderIdentifyResult(RecorderIdentifyStatus.None, 0, 0, $"");
        }
        ...
        // 第二层：ini 校验失败 = 疑似篡改
        if (_options.EnableSecret && !_bindingFile.VerifySignature(binding))
        {
            ...
            (recorder.DeptId is { } boundDept && dept is not null && boundDept != dept.Id))
        {
            ...
            var rewritten = new BindingInfo(
                ...
                boundAt,
                string.Empty);
            var signature = _options.EnableSecret ? _bindingFile.ComputeSignature(
