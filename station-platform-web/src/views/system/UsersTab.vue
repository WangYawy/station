...
import type { DeptItem, ImportResult, PagedResult, RoleItem, UserItem } from '../../api/types'
...
async function handleUserFile(file: { raw?: File }) {
  if (!file.raw) return
  const text = await file.raw.text()
  try {
    importResult.value = await apiText<ImportResult>('/imports/users', text)
  ...
  <div>
    <el-form inline>
      ...
      <el-form-item v-if="auth.hasPermission('user:manage')">
        ...
        <el-upload :auto-upload="false" :show-file-list="false" accept=".csv" :on-change="handleUserFile" style="display: inline-block; margin-left: 8px">
          <el-button type="primary">导入</el-button>
        </el-upload>
        <el-button @click="downloadUserTemplate()">下载模板</el-button>
      </el-form-item>
    </el-form>
